...
class CreateIndexTest extends WordSpec with Matchers {
  "CreateIndex" should {
    "create an index name without suffix when suffix not set" in {
      ...
      CreateIndex.getIndexName(kcql, Clock.systemUTC()) shouldBe "index_name"
    }
    ...
    "create an index name with suffix when suffix is set" in {
      ...
      CreateIndex.getIndexName(kcql, Clock.systemUTC()) shouldBe s"index_name_suffix_$formattedDateTime"
    }
  }
}
